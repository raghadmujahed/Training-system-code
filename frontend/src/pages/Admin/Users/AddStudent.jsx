  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        ...
        // الاستجابة قد تكون { data: [...] } أو المصفوفة مباشرة
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const students = rows.map((row) => ({
          ...
          department_id: departmentMap[row["القسم"]] || null,
          role_id: 5,
          status: "active",
        }));

        const validStudents = students.filter(s => s.name && s.email && s.university_id && s.department_id && s.major);
        if (validStudents.length === 0) {
          alert("بيانات غير كاملة: تأكد من وجود الاسم، البريد، الرقم الجامعي، القسم، التخصص");
          setBulkLoading(false);
          return;
        }
